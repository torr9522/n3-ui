...
install() {
    run_install_script "${XUI_RAW_BASE}/install_en.sh"
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    if [[ -f "${XUI_LOCAL_SHELL_SCRIPT}" ]]; then
        ...
        wget -O /usr/bin/x-ui -N --no-check-certificate "${XUI_RAW_BASE}/x-ui_en.sh"
    fi
    if [[ $? != 0 ]]; then
        echo ""
...
————————————————
 ...
 "
    show_status
    echo && read -p "please input a legal number[0-17],input 7 for checking login info:" num
